...
#
# Build WeeChat Relay Debian packages for a stable or development version.
#
...
Syntax: $0 devel|stable|<version> distro
       ...
       version  version to build: stable, devel or specific version
                ...
                for example: devel-2, stable-2, 1.0.0-2)
...
IMPORTANT: the current OS must match the distro, and the WeeChat Relay sources
...
Examples:
  ...
  $0 1.0.0 ubuntu/artful
  $0 1.0.0-2 ubuntu/zesty
  $0 test-patches
...
if [ "${version}" = "devel" ]; then
    ...
    # stable packages: (lib)weechatrelay-(-xxx)_X.Y.Z-1_arch.deb
    DEB_DIR="debian-stable"
    DEB_NAME="weechatrelay"
...
patch_file="${root_dir}/tools/debian/patches/${distro_type}_${distro_name}.patch"
